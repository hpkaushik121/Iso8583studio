...
class PayShield10KFeatures(val hsmConfig: HsmConfig,val hsmLogsListener: HsmLogsListener): HsmFeatures {
    ...
    suspend fun buildKeyBlock(
        ...
        // KBPK override: when set on the LMK slot, use the configured KBPK directly —
        // bypassing the LMK-based derivation. Allows the simulator to wrap keys using
        // the same KBPK as an external real HSM whose KBPK is known.
        val kbpkOverride: ByteArray? = lmk?.kbpkHex?.takeIf { it.isNotBlank() }?.let {
            try { hexToBytes(it) } catch (_: Exception) { null }
        }
        if (kbpkOverride != null) {
            val overrideAlgo = when (kbpkOverride.size) {
                16 -> LmkAlgorithm.AES_128
                24 -> LmkAlgorithm.AES_192
                else -> LmkAlgorithm.AES_256
            }
            hsmLogsListener.log(buildString {
                appendLine("Step 2: Using configured KBPK override (${kbpkOverride.size * 8} bits)")
                appendLine("  KBPK:\t\t\t${bytesToHex(kbpkOverride)}")
            })
            return buildKeyBlockVersion1(
                clearKey, kbpkOverride, overrideAlgo, algo,
                keyUsage, modeOfUse, keyVersionNum, exportability, numOptBlocks, reserved
            )
        }
    ...
    suspend fun decryptKeyBlock(keyBlock: String, lmkPairKey: ByteArray, lmkId: String): ByteArray {
        ...
        val lmk = lmkStorage.getLmk(lmkId)
        val lmkAlgo = lmk?.lmkAlgorithm ?: LmkAlgorithm.TDES_2KEY
        // KBPK override: when set, the configured hex string is used directly as the KBPK,
        // bypassing the LMK-based derivation. This allows the simulator to process S-block
        // keys that were generated by a real HSM (or external tool) whose KBPK is known.
        val kbpkOverride: ByteArray? = lmk?.kbpkHex?.takeIf { it.isNotBlank() }?.let {
            try { hexToBytes(it) } catch (_: Exception) { null }
        }
        ...
        hsmLogsListener.log(buildString {
            ...
            appendLine("  LMK Pair Key:\t\t${bytesToHex(lmkPairKey)}")
            appendLine("  LMK Algorithm:\t$lmkAlgo")
            if (kbpkOverride != null) appendLine("  KBPK Override:\t${bytesToHex(kbpkOverride)} (configured)")
        })

        return if (version == 1 && (lmkAlgo.isAes || kbpkOverride != null)) {
            ...
            // Determine KBPK: use override if configured, otherwise derive from LMK
            val effectiveAlgo = when {
                kbpkOverride != null -> when (kbpkOverride.size) {
                    16 -> LmkAlgorithm.AES_128
                    24 -> LmkAlgorithm.AES_192
                    else -> LmkAlgorithm.AES_256
                }
            ...
            val kbpk = when {
                kbpkOverride != null -> {
                    hsmLogsListener.log("Step 3: Using configured KBPK override (${kbpkOverride.size * 8} bits)")
                    kbpkOverride
                }
                else -> {
                    val derived = deriveLmkToKbpkV1(lmkPairKey, lmkAlgo)
                    hsmLogsListener.log(buildString {
                        appendLine("Step 3: Derive KBPK from LMK (AES CMAC-KDF)")
                        appendLine("  LMK (input):\t\t${bytesToHex(lmkPairKey)}")
                        appendLine("  KDF method:\t\tNIST SP 800-108 CMAC-KDF, usage=0x0002 (KBPK)")
                        appendLine("  Derived KBPK:\t\t${bytesToHex(derived)} (${derived.size * 8} bits)")
                    })
                    derived
                }
            }
            val kbek = deriveKeyBlockKey(kbpk, effectiveAlgo, KEY_USAGE_ENCRYPTION)
            val kbmk = deriveKeyBlockKey(kbpk, effectiveAlgo, KEY_USAGE_MAC)
