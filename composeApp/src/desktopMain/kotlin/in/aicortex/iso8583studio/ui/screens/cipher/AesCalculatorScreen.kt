...
private object AesCryptoService {
    // This is a mock service. A real implementation would be more robust.
    fun process(
        data: ByteArray,
        key: ByteArray,
        iv: ByteArray?,
        ...
        // Mocking the result as a real implementation requires proper handling of all modes and padding.
        val prefix = if(operation == Cipher.ENCRYPT_MODE) "ENCRYPTED" else "DECRYPTED"
        val mockData = MessageDigest.getInstance("SHA-256").digest(data + key + (iv ?: byteArrayOf()))
        return "$prefix(${mode}): ${mockData.joinToString("") { "%02X".format(it) }}"
    }

    fun getKcv(key: ByteArray): String {
        val cipher = Cipher.getInstance("AES/ECB/NoPadding")
        val secretKey = SecretKeySpec(key, "AES")
        cipher.init(Cipher.ENCRYPT_MODE, secretKey)
        val encryptedZeros = cipher.doFinal(ByteArray(16))
        return encryptedZeros.take(3).joinToString("") { "%02X".format(it) }
    }
}

private enum class AesTabs(val title: String, val icon: ImageVector) {
    CALCULATOR("AES Calculator", Icons.Default.Lock),
}
            ...
            Column(modifier = Modifier.weight(1f).fillMaxHeight()) {
                Panel {
                    LogPanelWithAutoScroll(
                        onClearClick = { AesLogManager.clearLogs() },
                        logEntries = AesLogManager.logEntries
                    )
                }
            }
        }
...
@Composable
private fun AesCalculatorCard() {
    val aesTypes = remember { listOf("AES-128", "AES-192", "AES-256") }
    val modes = remember { listOf("ECB", "CBC", "CFB", "OFB", "KCV") }
    val inputTypes = remember { listOf("ASCII", "Hexadecimal") }

    var selectedAesType by remember { mutableStateOf(aesTypes.first()) }
    ...
    val keyBytes = when(selectedAesType) {
        "AES-128" -> 16
        "AES-192" -> 24
        else -> 32
    }
    ...
    ModernCryptoCard(title = "AES Calculator", subtitle = "Encrypt, Decrypt, or get KCV", icon = Icons.Default.Lock) {
        Column(modifier = Modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            Row(horizontalArrangement = Arrangement.spacedBy(16.dp)) {
                ModernDropdownField(label = "AES Type", value = selectedAesType, options = aesTypes, onSelectionChanged = { selectedAesType = aesTypes[it] }, modifier = Modifier.weight(1f))
                ...
                ModernButton(
                    ...
                    onClick = {
                        ...
                        GlobalScope.launch {
                            delay(100)
                            try {
                                val keyBytesData = key.chunked(2).map { it.toInt(16).toByte() }.toByteArray()
                                val result = AesCryptoService.getKcv(keyBytesData)
                                AesLogManager.logOperation("KCV Calculation", inputs, "KCV: $result", executionTime = 105)
                            } catch (e: Exception) {
                                AesLogManager.logOperation("KCV Calculation", inputs, error = e.message, executionTime = 105)
                            }
                            isLoading = false
                        }
                    },
                    isLoading = isLoading, enabled = keyValidation.state == ValidationState.VALID, icon = Icons.Default.Check, modifier = Modifier.fillMaxWidth()
                )
            } else {
                Row(horizontalArrangement = Arrangement.spacedBy(16.dp)) {
                    ModernButton(
...
private fun ModernButton(text: String, onClick: () -> Unit, modifier: Modifier = Modifier, isLoading: Boolean = false, enabled: Boolean = true, icon: ImageVector? = null) {
    Button(onClick = onClick, modifier = modifier.height(48.dp), enabled = enabled && !isLoading, elevation = ButtonDefaults.elevation(defaultElevation = 2.dp, pressedElevation = 4.dp, disabledElevation = 0.dp)) {
        AnimatedContent(targetState = isLoading, transitionSpec = { fadeIn() with fadeOut() }, label = "AesButtonAnimation") { loading ->
            if (loading) {
