...
private enum class AtallaKeysTabs(val title: String, val icon: ImageVector) {
    KEY_ENCRYPTION("Key Encryption", Icons.Default.Lock),
    AKB_DECODE("AKB Decode", Icons.Default.LockOpen),
}

private object AtallaLogManager {
...
private object AtallaCryptoService {
    // This is a mock service.
    fun encryptKey(key: String, header: String, mfk: String): String {
        return "AKB: ${header.uppercase()}${key.hashCode().toString(16)}${mfk.hashCode().toString(16)}".uppercase()
    }

    fun decodeAkb(akb: String): Map<String, String> {
        val kcv = Random.nextInt(0, 0xFFFFFF).toString(16).uppercase().padStart(6, '0')
        val key = "DECRYPTED_KEY_${akb.take(4)}_${kcv}"
        return mapOf(
            "key" to key,
            "kcv" to kcv
        )
...
@Composable
fun AtallaKeysScreen( onBack: () -> Unit) {
    var selectedTabIndex by remember { mutableStateOf(0) }
    val tabList = AtallaKeysTabs.values().toList()
    val selectedTab = tabList[selectedTabIndex]

    Scaffold(
        topBar = { AppBarWithBack(title = "Atalla Keys Calculator", onBackClick = onBack) },
            ...
            Row(modifier = Modifier.fillMaxSize().padding(16.dp), horizontalArrangement = Arrangement.spacedBy(16.dp)) {
                Column(modifier = Modifier.weight(1f).fillMaxHeight(), verticalArrangement = Arrangement.spacedBy(16.dp)) {
                    AnimatedContent(
                        ...
                        label = "atalla_keys_tab_transition"
                    ) { tab ->
                        when (tab) {
                            AtallaKeysTabs.KEY_ENCRYPTION -> KeyEncryptionTab()
                            AtallaKeysTabs.AKB_DECODE -> AkbDecodeTab()
                        }
                    }
                }
                Column(modifier = Modifier.weight(1f).fillMaxHeight()) {
                    Panel {
                        LogPanelWithAutoScroll(
                            onClearClick = { AtallaLogManager.clearLogs() },
                            logEntries = AtallaLogManager.logEntries
                        )
                    }
                }
            }
...
@Composable
private fun KeyEncryptionTab() {
    var key by remember { mutableStateOf("") }
    var akbHeader by remember { mutableStateOf("") }
    var mfkKey by remember { mutableStateOf("") }

    val isFormValid = listOf(key, akbHeader, mfkKey).all { it.isNotBlank() } &&
            listOf(key, akbHeader, mfkKey).all { AtallaValidationUtils.validateHex(it, "Field").state == ValidationState.VALID }

    ModernCryptoCard(title = "Key Encryption", subtitle = "Encrypt a key into an Atalla Key Block (AKB)", icon = Icons.Default.Lock) {
        Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
            EnhancedTextField(value = key, onValueChange = { key = it.uppercase() }, label = "Key (Hex)", validation = AtallaValidationUtils.validateHex(key, "Key"))
            EnhancedTextField(value = akbHeader, onValueChange = { akbHeader = it.uppercase() }, label = "AKB Header (Hex)", validation = AtallaValidationUtils.validateHex(akbHeader, "AKB Header"))
            EnhancedTextField(value = mfkKey, onValueChange = { mfkKey = it.uppercase() }, label = "MFK Key (Hex)", validation = AtallaValidationUtils.validateHex(mfkKey, "MFK Key"))

            Spacer(Modifier.height(8.dp))
            ModernButton(
                text = "Encrypt Key",
                onClick = {
                    val inputs = mapOf("Key" to key, "AKB Header" to akbHeader, "MFK Key" to mfkKey)
                    val result = AtallaCryptoService.encryptKey(key, akbHeader, mfkKey)
                    AtallaLogManager.logOperation("Key Encryption", inputs, result)
                },
                enabled = isFormValid,
...
@Composable
private fun AkbDecodeTab() {
    val parityOptions = remember { listOf("None", "Odd", "Even") }

    var akb by remember { mutableStateOf("") }
    ...
    val isFormValid = AtallaValidationUtils.validateHex(akb, "AKB").state == ValidationState.VALID &&
            AtallaValidationUtils.validateHex(mfkKey, "MFK Key").state == ValidationState.VALID

    ModernCryptoCard(title = "AKB Decode", subtitle = "Decrypt an Atalla Key Block", icon = Icons.Default.LockOpen) {
        Column(modifier = Modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(16.dp)) {
            EnhancedTextField(value = akb, onValueChange = { akb = it.uppercase() }, label = "AKB (Atalla Key Block)", validation = AtallaValidationUtils.validateHex(akb, "AKB"), maxLines = 4)
            Row(verticalAlignment = Alignment.CenterVertically) {
                Checkbox(checked = checkKcv, onCheckedChange = { checkKcv = it })
                Text("Check KCV?", style = MaterialTheme.typography.body2)
            }
            EnhancedTextField(value = kcv, onValueChange = { kcv = it.uppercase() }, label = "KCV (S) (Optional)", validation = AtallaValidationUtils.validateHex(kcv, "KCV"))
            ...
            ModernButton(
                text = "Decode AKB",
                onClick = {
                    val inputs = mapOf("AKB" to akb, "MFK Key" to mfkKey, "Parity" to parity)
                    val resultData = AtallaCryptoService.decodeAkb(akb)
                    val resultString = "Decrypted Key: ${resultData["key"]}\nKCV: ${resultData["kcv"]}"
                    AtallaLogManager.logOperation("AKB Decode", inputs, resultString)
                },
                enabled = isFormValid,
...
private fun ModernButton(text: String, onClick: () -> Unit, modifier: Modifier = Modifier, isLoading: Boolean = false, enabled: Boolean = true, icon: ImageVector? = null) {
    Button(onClick = onClick, modifier = modifier.height(48.dp), enabled = enabled && !isLoading, elevation = ButtonDefaults.elevation(defaultElevation = 2.dp, pressedElevation = 4.dp, disabledElevation = 0.dp)) {
        AnimatedContent(targetState = isLoading, transitionSpec = { fadeIn() with fadeOut() }, label = "AtallaButtonAnimation") { loading ->
            if (loading) {
                Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.Center) {
