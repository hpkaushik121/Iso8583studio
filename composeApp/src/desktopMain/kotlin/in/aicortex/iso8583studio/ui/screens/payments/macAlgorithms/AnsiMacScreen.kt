...
object ANSIMACService {
    // NOTE: This is a placeholder for actual ANSI MAC cryptographic logic.
    fun generateMac(keyK: String, keyKPrime: String, data: String, algorithm: String, truncation: Int): String {
        // Mock logic: Combine keys based on algorithm, hash with data, and truncate.
        val combinedInput = buildString {
            append(keyK)
            if (algorithm.contains("X9.19") && keyKPrime.isNotBlank()) {
                append(keyKPrime)
            }
            append(data)
        }
...
@Composable
fun ANSIMACScreen(onBack: () -> Unit) {
    Scaffold(
        topBar = { AppBarWithBack(title = "ANSI MAC Calculator", onBackClick = onBack) },
        ...
        Row(
            ...
            Column(
                ...
                ANSIMacGenerationCard()
            }
            Column(modifier = Modifier.weight(1f).fillMaxHeight()) {
                Panel {
                    LogPanelWithAutoScroll(
                        onClearClick = { ANSIMACLogManager.clearLogs() },
                        logEntries = ANSIMACLogManager.logEntries
                    )
                }
            }
        }
...
@Composable
private fun ANSIMacGenerationCard() {
    // --- State Management ---
    val macAlgorithms = remember { listOf("ANSI MAC X9.9 (Wholesale MAC)", "ANSI MAC X9.19 (Retail MAC)") }
    var selectedAlgorithm by remember { mutableStateOf(macAlgorithms.first()) }

    var keyK by remember { mutableStateOf("") }
    var keyKPrime by remember { mutableStateOf("") }
    ...
    val showKeyKPrime by remember(selectedAlgorithm) {
        derivedStateOf { selectedAlgorithm.contains("X9.19") }
    }

    // --- Validation ---
    val keyKValidation = ANSIMACValidationUtils.validateHex(keyK, "Key (K)")
    val keyKPrimeValidation = ANSIMACValidationUtils.validateHex(keyKPrime, "Key (K')")
    val dataValidation = ANSIMACValidationUtils.validateHex(data, "Data")
    val truncationValidation = ANSIMACValidationUtils.validateNumeric(truncation, "Truncation Length")

    val isFormValid = remember(keyK, keyKPrime, data, truncation, showKeyKPrime) {
        keyK.isNotBlank() && data.isNotBlank() && truncation.isNotBlank() &&
                keyKValidation.state != ValidationState.ERROR &&
                dataValidation.state != ValidationState.ERROR &&
                truncationValidation.state != ValidationState.ERROR &&
                (!showKeyKPrime || (keyKPrime.isNotBlank() && keyKPrimeValidation.state != ValidationState.ERROR))
    }

    ModernCryptoCard(
        title = "ANSI MAC Generation",
        subtitle = "Generate X9.9 (Wholesale) or X9.19 (Retail) MAC",
        ...
        EnhancedTextField(keyK, { keyK = it }, "Key (K)", validation = keyKValidation)
        Spacer(Modifier.height(12.dp))

        // Dynamic Key (K') field
        AnimatedVisibility(
            visible = showKeyKPrime,
            ...
            Column {
                EnhancedTextField(keyKPrime, { keyKPrime = it }, "Key (K')", validation = keyKPrimeValidation)
                Spacer(Modifier.height(12.dp))
            }
        }
        ...
        ModernButton(
            ...
            onClick = {
                ...
                val inputs = mapOf(
                    ...
                    "Key (K)" to keyK,
                    "Key (K')" to if (showKeyKPrime) keyKPrime else "",
                    "Data" to data,
                ...
                GlobalScope.launch {
                    delay(150)
                    try {
                        val result = ANSIMACService.generateMac(keyK, keyKPrime, data, selectedAlgorithm, truncation.toInt())
                        ANSIMACLogManager.logOperation("MAC Generation", inputs, "Generated MAC: $result", executionTime = 155)
                    } catch (e: Exception) {
                        ANSIMACLogManager.logOperation("MAC Generation", inputs, error = e.message, executionTime = 155)
                    }
                    isLoading = false
                }
            },
...
private fun ModernButton(text: String, onClick: () -> Unit, modifier: Modifier = Modifier, isLoading: Boolean = false, enabled: Boolean = true, icon: ImageVector? = null) {
    Button(onClick = onClick, modifier = modifier.height(48.dp), enabled = enabled && !isLoading, elevation = ButtonDefaults.elevation(defaultElevation = 2.dp, pressedElevation = 4.dp, disabledElevation = 0.dp)) {
        AnimatedContent(targetState = isLoading, transitionSpec = { fadeIn() with fadeOut() }, label = "ANSIMACButtonAnimation") { loading ->
            if (loading) {
                Row(verticalAlignment = Alignment.CenterVertically) {
