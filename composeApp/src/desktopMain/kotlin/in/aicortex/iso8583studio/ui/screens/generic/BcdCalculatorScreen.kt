...
// --- BCD CONVERTER SCREEN ---

object BcdLogManager {
...
object BcdConversionService {
    fun encode(decimal: String): String {
        // Each decimal digit becomes a 4-bit nibble.
        // This mock represents it as a hex string. For unpacked BCD, "123" becomes hex "123".
        // For packed BCD, "123" becomes hex "0123". We'll mock unpacked.
        return decimal
    }

    fun decode(data: String, from: String): String {
        return when (from) {
            "Binary" -> data.chunked(4)
                .map { it.toInt(2) }
                .joinToString("")
            "Hexadecimal" -> data.map { it.toString() }.joinToString("")
            else -> throw IllegalArgumentException("Unsupported source format")
        }
    }
}

enum class BcdTabs(val title: String, val icon: ImageVector) {
...
fun BcdConverterScreen( onBack: () -> Unit) {
    ...
    Scaffold(
        topBar = { AppBarWithBack(title = "Binary Coded Decimal Converter", onBackClick = onBack) },
            ...
            Row(modifier = Modifier.fillMaxSize().padding(16.dp), horizontalArrangement = Arrangement.spacedBy(16.dp)) {
                Column(modifier = Modifier.weight(1f).fillMaxHeight(), verticalArrangement = Arrangement.spacedBy(16.dp)) {
                    PersistentTabContent(
                        ...
                        when(tab) {
                            BcdTabs.ENCODE -> EncodeBcdCard()
                            BcdTabs.DECODE -> DecodeBcdCard()
                        }
                    }
                }
                Column(modifier = Modifier.weight(1f).fillMaxHeight()) {
                    Panel {
                        LogPanelWithAutoScroll(
                            onClearClick = { BcdLogManager.clearLogs() },
                            logEntries = BcdLogManager.logEntries
                        )
                    }
                }
            }
...
@Composable
private fun EncodeBcdCard() {
    var data by remember { mutableStateOf("1234567890") }
    var isLoading by remember { mutableStateOf(false) }

    val validation = BcdValidationUtils.validateEncodeInput(data)
    val isFormValid = validation.state == ValidationState.VALID

    ModernCryptoCard(title = "Encode to BCD", subtitle = "Convert a decimal string to BCD format", icon = Icons.Default.Transform) {
        EnhancedTextField(
            value = data,
        ...
        ModernButton(
            ...
            onClick = {
                isLoading = true
                val inputs = mapOf("Input" to data)
                GlobalScope.launch {
                    delay(200)
                    try {
                        val result = BcdConversionService.encode(data)
                        BcdLogManager.logOperation("Encode to BCD", inputs, result = "BCD (Hex): $result", executionTime = 205)
                    } catch (e: Exception) {
                        BcdLogManager.logOperation("Encode to BCD", inputs, error = e.message, executionTime = 205)
                    }
                    isLoading = false
                }
            },
...
@Composable
private fun DecodeBcdCard() {
    val inputTypes = remember { listOf("Binary", "Hexadecimal") }
    var selectedInputType by remember { mutableStateOf(inputTypes.last()) }
    var data by remember { mutableStateOf("1234567890") }
    var isLoading by remember { mutableStateOf(false) }

    val validation = BcdValidationUtils.validateDecodeInput(data, selectedInputType)
    val isFormValid = validation.state != ValidationState.ERROR && data.isNotEmpty()

    ModernCryptoCard(title = "Decode from BCD", subtitle = "Convert BCD data to a decimal string", icon = Icons.Default.Transform) {
        ModernDropdownField(
            label = "Input Format",
        ...
        ModernButton(
            ...
            onClick = {
                isLoading = true
                val inputs = mapOf("Input Format" to selectedInputType, "Data" to data)
                GlobalScope.launch {
                    delay(200)
                    try {
                        val result = BcdConversionService.decode(data, selectedInputType)
                        BcdLogManager.logOperation("Decode from BCD", inputs, result = "Decimal: $result", executionTime = 205)
                    } catch (e: Exception) {
                        BcdLogManager.logOperation("Decode from BCD", inputs, error = e.message, executionTime = 205)
                    }
                    isLoading = false
                }
            },
...
private fun ModernDropdownField(label: String, value: String, options: List<String>, onSelectionChanged: (Int) -> Unit) {
    ...
    Box(modifier = Modifier.onGloballyPositioned { textFieldWidth = it.size.width }) {
        ...
        DropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }, modifier = Modifier.width(kotlin.with(density) { textFieldWidth.toDp() }).heightIn(max = 300.dp)) {
...
private fun ModernButton(text: String, onClick: () -> Unit, modifier: Modifier = Modifier, isLoading: Boolean = false, enabled: Boolean = true, icon: ImageVector? = null) {
    Button(onClick = onClick, modifier = modifier.height(48.dp), enabled = enabled && !isLoading, elevation = ButtonDefaults.elevation(defaultElevation = 2.dp, pressedElevation = 4.dp, disabledElevation = 0.dp)) {
        AnimatedContent(targetState = isLoading, transitionSpec = { fadeIn() with fadeOut() }, label = "BcdButtonAnimation") { loading ->
            if (loading) {
                Row(verticalAlignment = Alignment.CenterVertically) {
