...
// --- EMV DATA PARSER SCREEN ---

object EmvParserLogManager {
            ...
            Column(modifier = Modifier.weight(1f).fillMaxHeight()) {
                Panel {
                    LogPanelWithAutoScroll(
                        onClearClick = { EmvParserLogManager.clearLogs() },
                        logEntries = EmvParserLogManager.logEntries
                    )
                }
            }
        }
...
@Composable
private fun DataParserCard() {
    val tagTypes = remember { listOf("TVR", "CVR", "TLV") }
    var selectedTag by remember { mutableStateOf(tagTypes.first()) }
    ...
    if (showInfoDialog) {
        InfoDialog(
            title = "EMV Data Parsing",
            onDismissRequest = { showInfoDialog = false }
        ) {
            Text("This tool parses and interprets common EMV data structures.", style = MaterialTheme.typography.body2)
            Spacer(Modifier.height(8.dp))
            Text("TVR (Transaction Verification Result):", fontWeight = FontWeight.Bold)
            Text("...................................", style = MaterialTheme.typography.caption)
            Spacer(Modifier.height(8.dp))
            Text("CVR (Card Verification Results - Tag 9F10):", fontWeight = FontWeight.Bold)
            Text("A bit-mapped field indicating the results of card-based verification processes performed by the terminal.", style = MaterialTheme.typography.caption)
            Spacer(Modifier.height(8.dp))
            Text("TLV (Tag-Length-Value):", fontWeight = FontWeight.Bold)
            Text("A universal encoding scheme in EMV where data objects are represented by their tag, length, and value.", style = MaterialTheme.typography.caption)
        }
    }
    ...
    ModernCryptoCard(
        title = "EMV Data Parser",
        subtitle = "Decode TCT, CVR, or TLV data",
        ...
        Column(modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ModernDropdownField(
                label = "EMV Tag / Data Type",
                value = selectedTag,
                options = tagTypes,
                onSelectionChanged = { index -> selectedTag = tagTypes[index] }
            )
            ...
            EnhancedTextField(
                ...
                label = "Data Field",
                validation = if (data.isBlank()) ValidationResult(ValidationState.EMPTY, "Data field cannot be empty.") else ValidationResult(ValidationState.VALID),
                maxLines = 10
            )
            Spacer(Modifier.height(8.dp))
            ModernButton(
                ...
                onClick = {
                    isLoading = true
                    val inputs = mapOf("Tag Type" to selectedTag, "Input Data" to data)
                    GlobalScope.launch {
                        delay(500)
                        val mockResult = when (selectedTag) {
                            "TCT" -> "  - PAN: 541333******2222\n  - Expiry Date (YYMM): 2812\n  - Service Code: 201"
                            "CVR" -> "  - Byte 1: Offline data auth not performed\n  - Byte 2: PIN Try Limit Exceeded\n  - Byte 3: Issuer auth failed"
                            "TLV" -> "  - Tag 9F02: 000000012345 (Amount, Authorized)\n  - Tag 5F2A: 0840 (Transaction Currency Code)"
                            else -> "Parsed data will appear here."
                        }
                        EmvParserLogManager.logOperation("EMV Parsing", inputs, result = mockResult, executionTime = 510)
                        isLoading = false
                    }
                },
                isLoading = isLoading,
...
private fun ModernButton(text: String, onClick: () -> Unit, modifier: Modifier = Modifier, isLoading: Boolean = false, enabled: Boolean = true, icon: ImageVector? = null) {
    Button(onClick = onClick, modifier = modifier.height(48.dp), enabled = enabled && !isLoading, elevation = ButtonDefaults.elevation(defaultElevation = 2.dp, pressedElevation = 4.dp, disabledElevation = 0.dp)) {
        AnimatedContent(targetState = isLoading, transitionSpec = { fadeIn() with fadeOut() }, label = "ParserButtonAnimation") { loading ->
            if (loading) {
                Row(verticalAlignment = Alignment.CenterVertically) {
