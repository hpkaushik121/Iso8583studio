...
val thalesCommandDefinitions: List<ThalesCommandDefinition> = listOf(
    ...
    ThalesCommandDefinition(
        ...
        description = "Generate a random key and return it encrypted under LMK, optionally under ZMK.",
        ...
        requestFields = listOf(
            ...
            f("deriveKeyMode", "Derive Key Mode", FieldType.CODE, 1, default = "0",
                ...
                desc = "0 = DUKPT (BDK + KSN); 1 = ZKA (ZKA master key + options)",
                options = listOf(
            ...
            f("dukptMasterKey", "Key Block DUKPT Master Key", FieldType.HEX, 0,
                conds = a0DukptDerivePath,
                desc = "BDK encrypted under LMK (S-block or hex)"),
            f("ksnTdes", "KSN (3DES BDK)", FieldType.HEX, 15,
                ...
                desc = "15H: KSI(6H)+DID(8H) right-justified, F-padded on the left. Last hex char must be even. No transaction counter. Example: KSI=303950, DID=12342468 → F30395012342468"),
            f("ksnAes", "KSN (AES BDK)", FieldType.HEX, 16,
                ...
                desc = "16H: BDK_ID+DID right-justified, F-padded on the left. No transaction counter. No even constraint on last char. Example: BDK_ID=3039505, DID=12345678 → F303950512345678"),
            ...
            f("zkaMasterKey", "Key Block ZKA Master Key", FieldType.HEX, 0,
                conds = a0ZkaDerivePath,
                desc = "ZKA master key encrypted under LMK (S-block or hex)"),
            ...
            f("zkaRandomNumberInput", "ZKA RNDI", FieldType.HEX, 32,
                ...
                conds = a0ZkaDerivePath,
                desc = "32 hex digits (16 bytes) random input for ZKA derivation"),
            f("delimiterAfterLmkScheme", "Delimiter", FieldType.ASCII, 1, default = ";",
                cond = FieldCondition("mode", setOf("1", "B")),
                desc = "Separator before ZMK/TMK flag (e.g. ;)"),
            f("zmkTmkFlag", "ZMK/TMK Flag", FieldType.CODE, 1, default = "0",
                cond = FieldCondition("mode", setOf("1", "B")),
                options = listOf(CodeOption("0", "0 - ZMK"), CodeOption("1", "1 - TMK"))),
            f("zmkTmkBdk", "ZMK (TMK/Current BDK)", FieldType.HEX, 0,
                cond = FieldCondition("mode", setOf("1", "B")),
                desc = "ZMK/TMK/BDK encrypted under LMK"),
            ...
            f("lmkDelimiter", "Delimiter", FieldType.FLAG, 0, req = FieldRequirement.OPTIONAL,
                default = "%", desc = "Prefix % before 2-digit LMK pair"),
            f("lmkPairId", "LMK Identifier", FieldType.CODE, 2, req = FieldRequirement.OPTIONAL,
                ...
                desc = "LMK pair after % (BP-Tools: 03)"),
    ...
    ThalesCommandDefinition(
        ...
        requestFields = listOf(
            ...
            f("zmkTmkFlag", "ZMK/TMK Flag", FieldType.CODE, 1, default = "0",
                options = listOf(CodeOption("0", "0 - ZMK"), CodeOption("1", "1 - TMK"))),
            f("zmk", "ZMK/TMK", FieldType.HEX, 0,
                default = "S10096K0TN00E000353C5E6C6C42DDCD48EA0E2EF6ED4D0CF81F71EFEA8E7C0DFF7A1E68F14B72082CA2265868C04F806",
                desc = "Zone/Terminal Master Key encrypted under LMK (S-block or hex)"),
            f("key", "Key", FieldType.HEX, 0,
                ...
                desc = "Key to export, encrypted under LMK (S-block or hex)"),
            f("keyScheme", "Key Scheme(ZMK)", FieldType.CODE, 1, default = "S", options = keySchemeOptions,
                desc = "Encryption scheme for the output key under ZMK/TMK"),
    ...
    ThalesCommandDefinition(
        ...
        description = "Translate a PIN from BDK to BDK or ZPK encryption (3DES DUKPT)",
        category = CommandCategory.PIN_OPERATIONS,
        requestFields = listOf(
            f("sourceKeyFlag", "Source Key Flag", FieldType.CODE, 0,
                req = FieldRequirement.OPTIONAL, options = listOf(
                    CodeOption("", "(none) - TPK/BDK"),
                    CodeOption("~", "~ - ZPK"),
                ),
                desc = "Omit for TPK/BDK, ~ for ZPK"),
            f("sourceKey", "Source Key", FieldType.HEX, 0, desc = "Source PIN encryption key under LMK"),
            f("destinationKeyFlag", "Destination Key Flag", FieldType.CODE, 0,
                req = FieldRequirement.OPTIONAL, options = listOf(
                    CodeOption("", "(none) - TPK/BDK"),
                    CodeOption("~", "~ - ZPK"),
                ...
                desc = "Omit for TPK/BDK, ~ for ZPK"),
            f("destinationKey", "Destination Key", FieldType.HEX, 0, desc = "Destination PIN encryption key under LMK"),
            f("sourceKsnDescriptor", "Source KSN Descriptor", FieldType.HEX, 3,
                req = FieldRequirement.OPTIONAL, desc = "Required for DUKPT source"),
            f("sourceKsn", "Source KSN", FieldType.HEX, 0,
                req = FieldRequirement.OPTIONAL, desc = "Required for DUKPT source"),
            f("destinationKsnDescriptor", "Dest KSN Descriptor", FieldType.HEX, 0,
                req = FieldRequirement.OPTIONAL, desc = "Required for DUKPT destination"),
            f("destinationKsn", "Destination KSN", FieldType.HEX, 0,
                req = FieldRequirement.OPTIONAL, desc = "Required for DUKPT destination"),
            f("sourcePinBlock", "Source PIN Block", FieldType.HEX, 16),
            ...
            f("destinationPinBlockFormat", "Dest PIN Block Format", FieldType.CODE, 2, default = "01", options = pinBlockFormatOptions),
            f("pan", "PAN (Account Number)", FieldType.DEC, 12),
            f("destinationPanDelimiter", "Dest PAN Delimiter", FieldType.ASCII, 1,
                req = FieldRequirement.OPTIONAL, default = ";"),
            f("destinationPan", "Destination PAN", FieldType.DEC, 0, req = FieldRequirement.OPTIONAL),
            f("lmkIdentifier", "LMK Identifier", FieldType.CODE, 0, req = FieldRequirement.OPTIONAL, options = lmkIdentifierOptions),
        ),
        responseFields = listOf(
            f("errorCode", "Error Code", FieldType.DEC, 2),
    ...
    ThalesCommandDefinition(
        ...
        requestFields = listOf(
            ...
            f("outputFormatFlag", "Output Format Flag", FieldType.CODE, 1, default = "1", options = listOf(
                CodeOption("0", "0 - Binary"),
                CodeOption("1", "1 - Hex-Encoded Binary"))),
            f("keyType", "Key Type", FieldType.CODE, 3, default = "FFF", options = keyTypeOptions),
            f("key", "Key", FieldType.HEX, 0,
            ...
            f("msgLength", "Message Length", FieldType.HEX, 4, default = "0030",
                desc = "4H hex character count of message data (computed from message block in builder)"),
            f("messageBlock", "Message Block", FieldType.HEX, 0,
    ...
    ThalesCommandDefinition(
        ...
        requestFields = listOf(
            ...
            f("destinationModeFlag", "Destination Mode Flag", FieldType.CODE, 2, default = "00", options = listOf(
                ...
                CodeOption("04", "04 - CTR"))),
            f("inputFormatFlag", "Input Format Flag", FieldType.DEC, 1, default = "0"),
            f("outputFormatFlag", "Output Format Flag", FieldType.DEC, 1, default = "0"),
            ...
            f("encryptedData", "Encrypted Data", FieldType.HEX, 0),
            f("lmkIdentifier", "LMK Identifier", FieldType.CODE, 0, req = FieldRequirement.OPTIONAL,
                options = lmkIdentifierOptions),
        ),
        responseFields = listOf(
            f("errorCode", "Error Code", FieldType.DEC, 2),
            f("outputIV", "Output IV", FieldType.HEX, 0),
