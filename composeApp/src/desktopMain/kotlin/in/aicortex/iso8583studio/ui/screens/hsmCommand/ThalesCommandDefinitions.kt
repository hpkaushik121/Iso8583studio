...
val thalesCommandDefinitions: List<ThalesCommandDefinition> = listOf(
    ...
    ThalesCommandDefinition(
        ...
        description = "Translate a PIN block from DUKPT BDK encryption to another DUKPT BDK or ZPK. Supports S-block (KeyBlock) and variant-scheme keys.",
        category = CommandCategory.PIN_OPERATIONS,
        requestFields = listOf(
            f("bdk", "Source BDK", FieldType.HEX, 0,
                desc = "Source Base Derivation Key encrypted under LMK pair 28-29. S-block (e.g. S10096B0TN00...) or variant-scheme (U/T/X + 32H key hex)."),
            f("destKey", "Dest BDK / ZPK", FieldType.HEX, 0,
                desc = "Destination key encrypted under LMK. S-block BDK (e.g. S10096B0TN00...) or variant-scheme key."),
            f("srcKsnDescriptor", "Source KSN Descriptor", FieldType.HEX, 3, default = "609",
                desc = "3-hex descriptor for the source KSN. Defines counter bit allocation (e.g. 609 = 6+0+9 bits)."),
            f("srcKsn", "Source Key Serial Number", FieldType.HEX, 20,
                desc = "20H source KSN from the terminal. Contains BDK-ID, device-ID, and transaction counter."),
            f("dstKsnDescriptor", "Dest KSN Descriptor", FieldType.HEX, 3, default = "609",
                desc = "3-hex KSN descriptor for the destination BDK. Required for BDK-to-BDK translation."),
            f("dstKsn", "Dest Key Serial Number", FieldType.HEX, 20,
                desc = "20H destination KSN. Required for BDK-to-BDK translation. Used to derive the destination DUKPT session key."),
    ...
    ThalesCommandDefinition(
        ...
        requestFields = listOf(
            f("accountNumber", "Account Number", FieldType.DEC, 12, default = "123456789012"),
            f("pinLengthDelimiter", "Delimiter", FieldType.FLAG, 0, req = FieldRequirement.OPTIONAL,
                default = ";", desc = "Prefix ';' before 2-digit PIN LENGTH Identifier."),
